...
class PoseDisplay : public InternalFixture<::testing::Test>
{
};

/////////////////////////////////////////////////
TEST_F(PoseDisplay, GZ_UTILS_TEST_ENABLED_ONLY_ON_LINUX(Load))
{
  // Create app
  ...
  const char *pluginStr =
    "<plugin filename=\"PoseDisplay\">"
      "<gz-gui>"
        "<title>PoseDisplay!</title>"
      "</gz-gui>"
  ...
  EXPECT_TRUE(app->LoadPlugin("PoseDisplay",
  ...
  // Get plugin
  auto plugins = win->findChildren<sim::gui::PoseDisplay *>();
  ASSERT_EQ(plugins.size(), 1);

  auto plugin = plugins[0];
  EXPECT_EQ("PoseDisplay!", plugin->Title());
