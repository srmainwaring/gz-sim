...
class VisualizePose : public InternalFixture<::testing::Test>
{
};

/////////////////////////////////////////////////
TEST_F(VisualizePose, GZ_UTILS_TEST_ENABLED_ONLY_ON_LINUX(Load))
{
  // Create app
  ...
  const char *pluginStr =
    "<plugin filename=\"VisualizePose\">"
      "<gz-gui>"
        "<title>VisualizePose!</title>"
      "</gz-gui>"
  ...
  EXPECT_TRUE(app->LoadPlugin("VisualizePose",
  ...
  // Get plugin
  auto plugins = win->findChildren<sim::gui::VisualizePose *>();
  ASSERT_EQ(plugins.size(), 1);

  auto plugin = plugins[0];
  EXPECT_EQ("VisualizePose!", plugin->Title());
