...
void VisualizeForcesPrivate::OnRender()
{
  ...
  while (true)
  {
    ...
    //! @note the world pose obtained here will be at a time after the
    //        time at which the entity wrench message is created.
    math::Pose3d _worldPose;
    msgs::EntityWrench wrenchMsg;
    {
      ...
      std::tie(_worldPose, wrenchMsg) = this->queue.front();
      queue.pop();
    }
    ...
    auto worldPose = msgs::Convert(wrenchMsg.pose());
